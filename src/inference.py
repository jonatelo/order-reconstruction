...
class OrderInference:
    ...
    def monotonic_ranking(
        ...
        if feature_columns is None:
            feature_columns = features.select_dtypes(
                include=[np.number]
            ).columns.tolist()
        ...
        for col in feature_columns:
            ...
            if max_val > min_val:
                normalized[col] = (normalized[col] - min_val) / (
                    max_val - min_val
                )
            else:
                normalized[col] = 0
        ...
        sorted_indices = degradation_score.sort_values(
    ...
    @staticmethod
    def pca_based_ordering(
        features: pd.DataFrame, feature_columns: List[str] = None
    ) -> np.ndarray:
        """
    ...
    def weighted_ranking(
        features: pd.DataFrame, feature_weights: Dict[str, float]
    ) -> np.ndarray:
        """
        ...
        for feature_name in feature_weights.keys():
            ...
            if max_val > min_val:
                normalized[feature_name] = (
                    normalized[feature_name] - min_val
                ) / (max_val - min_val)
            else:
    ...
    def compute_spearman_footrule(
        predicted_order: np.ndarray, true_order: np.ndarray
    ) -> float:
        """
        ...
        predicted_ranks = {
            file_id: rank for rank, file_id in enumerate(predicted_order)
        }
    ...
    def compute_spearman_correlation(
        ...
        pred_ranks_list = [
            predicted_ranks[fid] for fid in predicted_ranks.keys()
        ]
