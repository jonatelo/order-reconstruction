...
class DataLoader:
    ...
    def load_file(self, file_path: Path) -> pd.DataFrame:
        ...
        try:
            ...
            if not all(col in df.columns for col in expected_cols):
                print(f"Warning: Expected columns {expected_cols} not found in {file_path}")
    ...
    def load_all(self, file_pattern: str = "file_*.csv", 
    ...
    def get_file_info(self, data: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        for file_id, df in data.items():
            info.append({
                ...
                'acceleration_mean': df['acceleration'].mean() if 'acceleration' in df.columns else None,
                'acceleration_std': df['acceleration'].std() if 'acceleration' in df.columns else None,
            })
