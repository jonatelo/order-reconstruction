...
class FeatureExtractor:
    ...
    def extract_time_domain_features(self, acceleration: np.ndarray) -> Dict[str, float]:
        """
        ...
        features = {
            ...
            'crest_factor': np.max(np.abs(acceleration)) / np.sqrt(np.mean(acceleration**2)),
    ...
    def extract_frequency_domain_features(self, acceleration: np.ndarray,
                                         fault_bands: List[float] = None) -> Dict[str, float]:
        """
        ...
        if fault_bands:
            for i, center_freq in enumerate(fault_bands):
                band_power = self._compute_band_power(fft_freq, fft_mag, 
                                                     center_freq, bandwidth=100)
    ...
    def extract_features_from_all(self, data: Dict[str, pd.DataFrame],
                                 fault_bands: List[float] = None) -> pd.DataFrame:
        """
    ...
    def _compute_band_power(self, freq: np.ndarray, magnitude: np.ndarray,
                           center_freq: float, bandwidth: float = 100) -> float:
        """
